...
if [ -n "${BENCH_ONLY:-}" ]; then
    ...
    timeout -k 60 900 "$PY" vast/benchmark.py --skip download,cpu --out /workspace/benchmark.json
...
# download test skipped: the training path never touches the HF CDN when the
# bank has the cache (and the bank test covers the path it does take). cpu
# test skipped: no jpeg decoding in this project. bank + disk + pcie + gpu ON:
# this workload is FLOP-bound and pulls 7.5 GB from Drive at boot.
if ! timeout -k 60 900 "$PY" vast/benchmark.py --skip download,cpu --gate "${THRESHOLDS_FILE:-vast/thresholds_gpt2.json}" --out /workspace/benchmark.json; then
