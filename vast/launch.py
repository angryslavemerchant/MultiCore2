...
REPO_URL  = "https://github.com/angryslavemerchant/MultiCore2.git"
REPO_DIR  = "MultiCore2"
...
# Default instance disk. 45 GB killed a predecessor run mid backward-compile
# with `OSError: [Errno 28] No space left on device` in /tmp/torchinductor_root
# (the inductor cache is unbounded across relaunches). MultiCore2's budget:
#
#   docker image                                  ~20 GB
#   token cache, 5 shards                           7.5
#   HF parquet staged if the bank pull fails      ~12
#   inductor/triton cache (grows per recompile)     5-15
#   124M checkpoints, fp32 + 2 Adam moments =
#     ~1.5 GB each x {latest, best}                  3
#
# ~58 GB before slack; 100 leaves room and matches more offers than 150 did.
DISK_GB   = 100
...
GPU_PROFILES = {
    "cheap": {  # a 4090 does the 2.5B-token baseline in ~6-8 h (~$2-3)
        "gpu_name": "RTX_4090",
        "max_dph": 0.35,
        "train_frag": "",
    },
...
def search_offers(gpu: str, max_dph: float, inet: int = 500, limit: int = 40,
    ...
    # MultiCore2 pulls a 7.5 GB token cache at boot (inet_down floor keeps
    # that quick) and dataloading is a memmap gather on one core; 16 GB RAM
    # and 4 cores are ample. The FLOP floor is enforced by the boot gate,
    # not the offer query.
...
def cmd_launch(args):
    ...
    if args.smoke:
        args.train_args = "--smoke --wandb --run-name smoke_test"
    ...
    print("  wandb:   project 'multicore2' - run appears once training starts")
